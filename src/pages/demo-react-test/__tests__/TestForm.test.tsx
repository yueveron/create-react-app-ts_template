...
describe('Formik Test Cases', () => {
  ...
  test('test Formik form', async () => {
    ...
    const testFormEle = screen.getByTestId('test-form');
    await waitFor(() => {
      expect(testFormEle).toHaveFormValues({
        firstName: 'John',
      });
    });

    await waitFor(() => {
      expect(container).toMatchSnapshot('test-formik');
    });
  }, 120000);
});
